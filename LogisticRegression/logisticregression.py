...
class LogisticRegression:
    ...
    def train(self, training_data: List[np.ndarray], training_labels: List[int], 
          dev_data: List[np.ndarray], dev_labels: List[int], log_interval=2500) -> None:
        """Εκπαίδευση του αλγορίθμου με καταγραφή των επιδόσεων ανά αριθμό παραδειγμάτων"""
        
        self.weights = self.initialize_weights(len(training_data[0]))
        train_precision, train_recall, train_f1 = [], [], []
        dev_precision, dev_recall, dev_f1 = [], [], []
        sample_counts = []  # Λίστα που κρατά τον αριθμό των δειγμάτων

        total_samples = 0  # Καταμέτρηση του αριθμού των δειγμάτων που έχουν επεξεργαστεί

        for epoch in range(self.epochs):
            total_loss = 0
            data_with_labels = list(zip(training_data, training_labels))
            ...
            for (x, y) in zip(shuffled_data, shuffled_labels):
                ...
                y_pred = self.sigmoid(t)
                example_loss = -y * np.log(y_pred) - (1 - y) * np.log(1 - y_pred)
                total_loss += example_loss
                self.weights += self.learning_rate * (y - y_pred) * x - self.learning_rate * self.lambda_ * self.weights
                
                total_samples += 1  # Αυξάνουμε τον μετρητή των δειγμάτων

                # Καταγράφουμε τις μετρικές κάθε log_interval δείγματα
                if total_samples % log_interval == 0:
                    ...
                    sample_counts.append(total_samples)  # Αποθηκεύουμε το πλήθος των δειγμάτων

            regularization_term = (self.lambda_ / 2) * np.sum(self.weights ** 2)
        ...
        plt.ylabel("Score")
        plt.title("Learning Curves based on Training Examples")
        """
    ...
    def evaluate(self, test_data: List[np.ndarray], test_labels: List[str]) -> None:
        """Aksiologhsh tou algorithmou epeita apo to training"""
        test_preds = [self.predict(x) for x in test_data]  #provlepseis sta test data me vash ta telika varh

        precision = precision_score(test_labels, test_preds, average=None)  #upologismos metrics epidoshs
...
def main():
    ...
    model.train(training_data, training_labels, dev_data, dev_labels) #ekpaideush algorithmou

    print("Evaluating model...")
    model.evaluate(test_data, test_labels)  #aksiologhsh algorithmou
